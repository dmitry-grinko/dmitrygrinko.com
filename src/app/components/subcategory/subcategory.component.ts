...
export class SubcategoryComponent implements OnInit {
  ...
  private loadCategoryData() {
    this.blogService.getCategoryTree().subscribe(categories => {
      ...
      if (category) {
        this.categoryDisplayName = category.name;
        const subcategory = category.subcategories.find(sub => sub.slug === this.subcategorySlug);
        this.subcategoryDisplayName = subcategory ? subcategory.name : this.subcategorySlug;
      }
    });
  }

  private loadPosts() {
    this.blogService.getPostsBySubcategory(this.categorySlug, this.subcategorySlug).subscribe(posts => {
      this.posts = posts;
  ...
  getSubcategoryDisplayName(): string {
